...
public sealed class ReleaseToolTests
{
    ...
    public void WorkflowBuildsExactRedLeafToolThenProducesAndBridgesUnsignedGenericCandidate()
    {
        ...
        Assert.Contains("$redBambooRoot = (Resolve-Path -LiteralPath '../redbamboo-packages').Path", workflow, StringComparison.Ordinal);
        Assert.Contains("-p:RedBambooPackagesRoot=\"$redBambooRoot\"", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("GH_TOKEN: ${{ secrets.CROSS_REPO_TOKEN || github.token }}", workflow, StringComparison.Ordinal);
        Assert.DoesNotContain("GH_TOKEN: ${{ github.token }}", workflow, StringComparison.Ordinal);
